    ...
    partial struct Job : IJobEntity
    {
        ...
        public void Execute(in LocalTransform2D local, ref LocalTransform world)
        {
...

            {
                ...
                position = point;
                rotation = quaternion.LookRotation(tangent, pointNormal);
                normal = pointNormal;
            }
            
            //TorusMapper.GetPositionAndUpRotation(local.Position, ref position, ref rotation, ref normal);
            world = LocalTransform.FromPositionRotation(position, math.mul(rotation, quaternion.AxisAngle(normal, local.Rotation)));
        }
    }
}
