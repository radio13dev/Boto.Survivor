...
public class TorusTerrainTool : MonoBehaviour
{
    ...
    [EditorButton]
    public void GenerateCylinderMesh(float thickness, int ringSegments, int tubeSegments)
    {
        if (!m_GeneratedMesh)
        {
            m_Mesh = new Mesh();
        ...
        TorusMeshGenerator.GenerateCylinderMesh(thickness, ringSegments, tubeSegments, TorusMeshGenerator.Axis.y, out var verts, out var tris, out var normals, out var uvs);
        m_Mesh.SetVertices(Array.ConvertAll(verts, v => (Vector3)v));
        m_Mesh.SetTriangles(tris, 0);
        m_Mesh.SetNormals(Array.ConvertAll(normals, v => (Vector3)v));
        m_Mesh.SetUVs(0, Array.ConvertAll(uvs, v => (Vector2)v));
