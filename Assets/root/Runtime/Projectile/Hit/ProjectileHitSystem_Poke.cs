...
[BurstCompile]
public partial struct ProjectileHitSystem_Poke : ISystem
{
    ...
    partial struct Job : IJobEntity
    {
        ...
        public void Execute([ChunkIndexInQuery] int Key, in DynamicBuffer<ProjectileHitEntity> hits, in Poke Poke)
        {
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                if (e != Entity.Null)
                {
                    ecb.AppendToBuffer(Key, e, Pending.Poke(Poke.Value));
