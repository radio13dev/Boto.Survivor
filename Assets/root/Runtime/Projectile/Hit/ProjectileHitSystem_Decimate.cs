...
[BurstCompile]
public partial struct ProjectileHitSystem_Decimate : ISystem
{
    ...
    partial struct Job : IJobEntity
    {
        ...
        public void Execute([ChunkIndexInQuery] int Key, in DynamicBuffer<ProjectileHitEntity> hits, in Decimate Decimate)
        {
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                if (e != Entity.Null)
                {
                    ecb.AppendToBuffer(Key, e, Pending.Decimate(Decimate.Value));
